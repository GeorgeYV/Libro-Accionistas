...
export default function Canje() {
  ...
  const addOperacion = async () => {
    try {
      ...
      const operacion = {
        ope_fecha: fecha,
        ope_tipo: 2,
        ope_acciones: valCedente.acc_cantidad_acciones,
        ope_estado: 0,
  ...
  async function fetchTitulos(accId) {
    ...
    const apiData = await API.graphql({ query: listTitulos, variables: { filter: filtro, limit: 1000 } });
        ...
        <Grid container style={{ width: '90%' }}>
          ...
          <Grid item xs={4} >
            <div className={classes.cedente}>
              ...
              />
              {valCedente.acc_cantidad_acciones > 0 &&
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                  <Typography variant='caption' color='primary'>
                    Total Acciones Ordinarias: {valCedente.acc_cantidad_acciones}
                  </Typography>
                </div>
              }
            </div>
                  ...
                  {titulos.map(item => (
                    <ListItem key={item.titulo}>
                      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', width: '100%' }}>
                        <ListItemText style={{ flex: 1 }}>{item.updatedAt}</ListItemText>
                        <ListItemText style={{ flex: 1 }}>{item.id}</ListItemText>
                        <ListItemText style={{ flex: 1 }}>{item.tit_acciones}</ListItemText>
                      </div>
                    </ListItem>
                  ))}
                </List>
          ...
          <Grid item xs={4} container>
            <div style={{ display: 'flex', flexDirection: 'column', marginTop: 20, paddingLeft: 10 }}>
              ...
              <label htmlFor="upload-photo1">
                ...
                <Button startIcon={<CloudUploadOutlinedIcon />} variant='outlined' component="span" color="primary" size='medium' style={{ textTransform: 'none', }}>Título Ordinario</Button>
          ...
          <Grid item xs={12} container direction='row' justifyContent='center' style={{ paddingTop: 20 }} >
            <Button
              ...
              disabled={valCedente.acc_cantidad_acciones > 0 ? false : true}
            >
