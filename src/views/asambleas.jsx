...
import {
  Grid, Typography, Button, ListItem, ListItemText, ListSubheader, List, Tooltip, Chip, TextField, LinearProgress,
  FormControl, RadioGroup, FormControlLabel, Radio, Box, Tabs, Tab, IconButton, Checkbox, InputLabel, Select, MenuItem, Card,
  Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, ListItemIcon, Snackbar, CircularProgress, Paper, Divider, StepIcon
...
export default function Asambleas() {
  ...
  const addAccionista = async () => {
    try {
      ...
      console.log("Datos Accionista", valCedente);
      ...
      const accionista = {
        ...
        presente: 'false',
        representanteNombre: nombreRepresentante ? nombreRepresentante : '',
        representanteDocumento: identificacionRepresentante ? identificacionRepresentante : '',
        representanteDI: IDRepresentante.filename ? IDRepresentante.filename : '',
        ...
        <Dialog open={openAccionistas} onClose={handleClose} aria-labelledby="form-dialog-title" maxWidth='md'  >
          ...
          <DialogContent style={{ height: '500px', width: '900px' }}>
            ...
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
              <Tabs value={value} onChange={handleChangeTab} aria-label="">
            ...
            <TabPanel value={value} index={2}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'space-between', width: '100%', marginBottom: '20px' }}>
                ...
                <FormControl style={{ width: '100%', marginBottom: 20 }}>
                  ...
                  >
                    {asambleaSeleccionada.votacionTema1 && <MenuItem value={"1"}>{asambleaSeleccionada.votacionTema1}</MenuItem>}
                    {asambleaSeleccionada.votacionTema2 && <MenuItem value={"2"}>{asambleaSeleccionada.votacionTema2}</MenuItem>}
                    {asambleaSeleccionada.votacionTema3 && <MenuItem value={"3"}>{asambleaSeleccionada.votacionTema3}</MenuItem>}
                    {asambleaSeleccionada.votacionTema4 && <MenuItem value={"4"}>{asambleaSeleccionada.votacionTema4}</MenuItem>}
                    {asambleaSeleccionada.votacionTema5 && <MenuItem value={"5"}>{asambleaSeleccionada.votacionTema5}</MenuItem>}
                    {asambleaSeleccionada.votacionTema6 && <MenuItem value={"6"}>{asambleaSeleccionada.votacionTema6}</MenuItem>}
                    {asambleaSeleccionada.votacionTema7 && <MenuItem value={"7"}>{asambleaSeleccionada.votacionTema7}</MenuItem>}
                    {asambleaSeleccionada.votacionTema8 && <MenuItem value={"8"}>{asambleaSeleccionada.votacionTema8}</MenuItem>}
                    {asambleaSeleccionada.votacionTema9 && <MenuItem value={"9"}>{asambleaSeleccionada.votacionTema9}</MenuItem>}
                    {asambleaSeleccionada.votacionTema10 && <MenuItem value={"10"}>{asambleaSeleccionada.votacionTema10}</MenuItem>}
                  </Select>
                </FormControl>
